        ...
        <div class="form-group">
            ...
            <select name="menu_category_id" id="menu_category_id" class="form-control" required>
                @foreach($all_categories as $category)
                    <option value="{{ $category->id }}" {{ $category->id == old('menu_category_id', $product->menu_category_id) ? 'selected' : '' }}>
                        {{ $category->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            @foreach($customGroups as $group)
                <div class="form-check">
                    <input type="checkbox" name="custom_groups[{{ $loop->index }}][id]" value="{{ $group->id }}" class="form-check-input"
                        {{ $product->customGroups->contains($group->id) ? 'checked' : '' }}>
